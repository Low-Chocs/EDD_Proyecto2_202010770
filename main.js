...
//Merkle
class data_node {
    constructor(_client_name, _movie_name) {
...
class merkle_tree {
    ...
    add(_client_name, _movie_name) {
        this.datablock.push(new data_node(_client_name, _movie_name));
    }

    create_tree(exp) {
        this.tophash = new hash_node(0)
        this._create_tree(this.tophash, exp)
    }

    _create_tree(tmp, exp) {
        if (exp > 0) {
            tmp.left = new hash_node(0)
            tmp.right = new hash_node(0)
            this._create_tree(tmp.left, exp - 1)
            this._create_tree(tmp.right, exp - 1)
        }
    }

    genHash(tmp, n) { // postorder travel
        if (tmp != null) {
            this.genHash(tmp.left, n)
            this.genHash(tmp.right, n)

            if (tmp.left == null && tmp.right == null) {
                tmp.left = this.datablock[n - index2--]
                tmp.hash = sha256(tmp.left.client_name + " - " + tmp.left.movie_name);
            } else {
    ...
    auth() {
        ...
        for (var i = this.datablock.length; i < Math.pow(2, exp); i++) {
            this.datablock.push(new data_node(1))
        }
        index2 = Math.pow(2, exp)
        this.create_tree(exp)
    ...
    dotgen(tmp) {
        if (tmp != null) {
          if (tmp.left != null){
            if (tmp.left instanceof data_node) {
              this.dot += '"'+tmp.left.value+'" [color=gray] -> "0x'+tmp.hash+'";'
            }
          }
          if (tmp.left instanceof hash_node) {
    ...
    preorder(tmp){
        if (tmp != null) {
            if (tmp instanceof data_node) {
                console.log(tmp.client_name, tmp.movie_name);
            } else {
